...
// ── GET /cabinet/demandes/:expertUserId — Demandes en attente de validation ──
...
app.post('/cabinet/demandes/valider', async (req, res) => {
  try {
    ...
    // Récupérer infos PME pour notifier
    const { data: company } = await supabase
      .from('companies').select('company_name, email').eq('id', invite.company_id).single();

    // Notifier la PME par email
    try {
      const emailService = require('./services/email.service');
      const { data: expertUser } = await supabase
        .from('users').select('full_name').eq('id', expert_user_id).single();
      await emailService.envoyerEmail({
        ...
        htmlContent: `<div style="font-family:Arial,sans-serif;max-width:580px;margin:0 auto">
          ...
          <div style="padding:24px 32px;background:#fff;border:1px solid #E2EDE8;border-top:none;border-radius:0 0 8px 8px">
            <p style="color:#2D3A35">Bonne nouvelle ! <strong>${expertUser?.full_name || 'Votre cabinet expert'}</strong> a accepté votre demande. Vous êtes maintenant dans leur portefeuille clients.</p>
    ...
    } catch(e) { console.warn('Email notif PME acceptée:', e.message); }
...
app.post('/cabinet/demandes/refuser', async (req, res) => {
  try {
    ...
    const { data: invite } = await supabase.from('invites').select('id, status, invited_by, email, company_id').eq('id', invite_id).single();
    ...
    try {
      ...
      await emailService.envoyerEmail({
        to: invite.email,
        subject: 'Votre demande n\'a pas été acceptée — H-Compta AI',
    ...
    } catch(e) { console.warn('Email notif PME refusée:', e.message); }
...
app.post('/inscription/pme', async (req,res) => {
  try {
    ...
    // Si la PME vient d'un lien cabinet → créer une DEMANDE EN ATTENTE de validation
    if (expert_id) {
      try {
        const { data: expertUser } = await supabase
          .from('users')
        ...
        if (expertUser) {
          // Créer la demande dans invites avec status 'pending_pme'
          // company_id = cabinet expert (pas la PME — c'est la PME qui demande à rejoindre)
          // on stocke expert_id dans invited_by, company_id = nouvelle PME
          await supabase.from('invites').insert([{
            company_id:  company.id,          // PME qui demande
            email:       email.toLowerCase(), // email de la PME
            role:        'PME_CLIENT',        // rôle demandé
            token:       require('crypto').randomBytes(16).toString('hex'),
            status:      'pending_pme',       // en attente de validation cabinet
            invited_by:  expert_id,           // expert qui a envoyé le lien
            expires_at:  new Date(Date.now() + 30*24*60*60*1000).toISOString(), // 30 jours
          }]);
          console.log(`📥 Demande portefeuille : PME ${company.id} → expert ${expert_id}`);
          ...
          try {
            ...
            await emailService.envoyerEmail({
              to: expertUser.email,
              subject: `${company_name} demande à rejoindre votre portefeuille`,
              htmlContent: `<div style="font-family:Arial,sans-serif;max-width:580px;margin:0 auto">
                <div style="background:#0D2B22;padding:24px 32px;border-radius:8px 8px 0 0">
                  <h2 style="color:#fff;margin:0">H-Compta AI — Nouvelle demande</h2>
                </div>
                <div style="padding:24px 32px;background:#fff;border:1px solid #E2EDE8;border-top:none;border-radius:0 0 8px 8px">
                  <p style="color:#2D3A35">Bonjour ${expertUser.full_name || 'Maître'},</p>
                  <p style="color:#2D3A35"><strong>${company_name}</strong> (${pays}) souhaite rejoindre votre portefeuille cabinet sur H-Compta AI.</p>
...
app.post('/connexion', async (req,res) => {
  try {
    ...
    // Si PME déjà inscrite arrive via lien cabinet → créer demande portefeuille
    if (expert_id && company && ['PME_OWNER','COLLABORATOR'].includes(user.role)) {
      try {
        // Vérifier qu'il n'y a pas déjà une demande pending
        const { data: existingReq } = await supabase.from('invites')
          .select('id, status')
          .eq('company_id', company.id)
          .eq('invited_by', expert_id)
          .in('status', ['pending_pme', 'accepted'])
          .single();
        ...
        if (!existingReq) {
          ...
          if (expertUser) {
            ...
            try {
              ...
              await emailService.envoyerEmail({
                to: expertUser.email,
                subject: `${company.company_name} demande à rejoindre votre portefeuille`,
                htmlContent: `<div style="font-family:Arial,sans-serif;max-width:580px;margin:0 auto">
                  <div style="background:#0D2B22;padding:24px 32px;border-radius:8px 8px 0 0">
                    <h2 style="color:#fff;margin:0">H-Compta AI — Nouvelle demande</h2>
                  </div>
                  <div style="padding:24px 32px;background:#fff;border:1px solid #E2EDE8;border-top:none;border-radius:0 0 8px 8px">
                    <p style="color:#2D3A35"><strong>${company.company_name}</strong> (${user.country}) souhaite rejoindre votre portefeuille cabinet.</p>
                    <a href="https://hcompta-ai.com/dashboard_expert_comptable_v2.html" style="display:inline-block;background:#0D2B22;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;margin-top:8px">Voir la demande →</a>
                    <p style="color:#9DB8AC;font-size:12px;margin-top:24px">H-Compta AI · hcompta-ai.com</p>
                  </div>
                </div>`,
              });
            } catch(e) { console.warn('Email notif expert connexion:', e.message); }
          }
        }
      } catch(reqErr) { console.warn('Demande portefeuille connexion:', reqErr.message); }
    }
    // Générer le JWT
    const token = genererToken({
      user_id:    user.id,
    ...
    return res.json({
      ...
      country:         user.country,
      company,
      demande_envoyee: !!(expert_id && company && ['PME_OWNER','COLLABORATOR'].includes(user.role)),
